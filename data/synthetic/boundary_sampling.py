...
def boundary_sampling(sigma,path):
    try:
        ...
        out_file = os.path.dirname(path) + '/boundary_{}_samples.npz'.format(sigma)
        ...
        np.savez(out_file, points=boundary_points, df = df)
    ...
    del mesh, df, boundary_points, points#,labels
    gc.collect()
